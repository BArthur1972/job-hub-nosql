...
function JobSeekerDashboard() {
  ...
  useEffect(() => {
    ...
    fetchJobs();
  }, [getAllJobListing, getCompanyById, user._id]);
  ...
  const filteredJobs = jobList.filter((job) => {
    ...
    const hasSkills = user.skills.some((skill) => {
      jobSkills.some((jobSkill) => jobSkill.trim() === skill)
      return jobSkills.includes(skill);
    });

    const hasTitle = user.experience.some((experience) =>
      jobTitle.includes(experience.role)
    );
            ...
            <Col xs={12}>
              ...
              <Row xs={1} md={2} lg={3} xl={4} className="g-4">
                {filteredJobs.length !== 0 ? (
                  ...
                  <p>
                    No jobs found that match your profile. Try searching on the
                    Jobs Page.
                  </p>
                )}
              </Row>
