...
function JobApplication() {
  ...
  const [getUserJobApplications] = useGetApplicationsByJobSeekerIdMutation();
  ...
  useEffect(() => {
    const fetchApplications = async () => {
      ...
      setError(null);
      try {
        const response = await getUserJobApplications(user._id);
        if (response.data) {
    ...
    fetchApplications();
  }, [getUserJobApplications, getCompanyById, user._id]);
