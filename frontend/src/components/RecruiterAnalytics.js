...
function RecruiterAnalytics() {
    ...
    useEffect(() => {
        ...
        fetchApplicantStatusCounts(user._id);
        fetchEmploymentTypeCounts(user._id);
    ...
    const employmentTypeData = Object.entries(employmentTypeCounts).filter(([_, value]) => value > 0).map(([name, value]) => ({
        name,
        value,
    }));
    ...
    // Define custom colors for each slice in the pie chart
    const COLORS = ['#0088FE', '#00C49F', '#FFBB28', '#FF8042'];

    const checkForData = () => {
        return !applicantStatusData.every(({ name, value }) => name[value] === 0) && !employmentTypeData.every(({ name, value }) => name[value] === 0);
    }
