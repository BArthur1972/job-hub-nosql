        ...
        return new Promise((resolve, reject) => {
            ...
            dbConnection.query(query, async (err, result) => {
                if (err) {
                    ...
                    const jobListingsWithCompanyNames = await Promise.all(result.map(async (jobListing) => {
                        const company = await Company.getCompanyById(jobListing.companyID);
                    ...
                    const jobListingsWithApplicantCounts = await Promise.all(jobListingsWithCompanyNames.map(async (jobListing) => {
                        const result = await Application.getNumberOfApplicants(jobListing.jobID);
                    ...
                    jobListingsWithApplicantCounts.forEach((jobListing) => {
                        jobListing.postingDate = new Date(jobListing.postingDate).toISOString().slice(0, 10)
                        jobListing.expirationDate = new Date(jobListing.expirationDate).toISOString().slice(0, 10)
                    });
            ...
            dbConnection.query(query, async (err, result) => {
                if (err) {
                    ...
                    await Promise.all(result.map(async (application) => {
                        return Application.deleteApplication(application.seekerID, jobListingId);
                    }));
